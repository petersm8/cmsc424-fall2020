    ...
    def get_next(self):
        if self.jointype == self.INNER_JOIN:
            ...
            for r in self.right_child.get_next():
                key = r.getAttribute(self.right_attribute)
                if key in hashtable:
                    hashtable[r.getAttribute(self.right_attribute)].append([r, False])
                else:
                    hashtable[r.getAttribute(self.right_attribute)] = [[r, False]]
            # Then, for each tuple in the left input, we look for matches and output those
            # Using "yield" significantly simplifies this code
            for l in self.left_child.get_next():
                key = l.getAttribute(self.left_attribute)
                ...
                if key in hashtable:
                    for r in hashtable[key]:
                        output = list(l.t)
                        output.extend(list(r[0].t))
                        r[1] = True
                        yield Tuple(None, output)
                else:
                    for r in range(0, len(self.right_child.relation.schema)):
                        output.append('NULL')
                    yield Tuple(None, output)
...
class GroupByAggregate(Operator):
    ...
    def update_aggregate(aggregate_function, current_aggregate, new_value):
        if aggregate_function == GroupByAggregate.COUNT:
            ...
            if current_aggregate is None:
                ...
                current_aggregate[0][1] += 1
                current_aggregate[0][0] = current_aggregate[0][0] + int(new_value)
    ...
    def final_aggregate(aggregate_function, current_aggregate):
        if aggregate_function in [GroupByAggregate.COUNT, GroupByAggregate.SUM, GroupByAggregate.MIN,
                                  GroupByAggregate.MAX]:
            return current_aggregate
        elif aggregate_function == GroupByAggregate.AVERAGE:
            if current_aggregate is None:
                return current_aggregate
            return current_aggregate[0][0] / current_aggregate[0][1]
        elif aggregate_function == GroupByAggregate.MEDIAN:
            if current_aggregate is None:
                return current_aggregate
            current_aggregate.sort()
            return current_aggregate[int(math.floor(len(current_aggregate) / 2))]
        elif aggregate_function == GroupByAggregate.MODE:
            if current_aggregate is None:
                return current_aggregate
            return max(set(current_aggregate), key=current_aggregate.count)
        else:
            raise ValueError("No such aggregate")
    ...
    def get_next(self):
        if self.group_by_attribute is None:
            ...
            for t in self.child.get_next():
                aggr = GroupByAggregate.update_aggregate(self.aggregate_function, aggr,
                                                         t.getAttribute(self.aggregate_attribute))
            ...
            for t in self.child.get_next():
                ...
                aggrs[g_attr] = GroupByAggregate.update_aggregate(self.aggregate_function, aggrs[g_attr],
                                                                  t.getAttribute(self.aggregate_attribute))
    ...
    def get_next(self):
        ...
        for r in self.right_child.get_next():
            if r.t in right_hashtable:
                ...
                right_hashtable[r.t] = 1
        for l in self.left_child.get_next():
            if l.t in left_hashtable:
                left_hashtable[l.t] += 1
            else:
                left_hashtable[l.t] = 1

        if self.keep_duplicates:
            if len(left_hashtable.items()) == 0 or len(right_hashtable.items()) == 0:
                return None
            else:
                for key in left_hashtable:
                    if key in right_hashtable:
                        l_tuples = left_hashtable[key]
                        r_tuples = right_hashtable[key]
                        if l_tuples > r_tuples:
                            for i in range(0, r_tuples):
                                yield Tuple(None, key)
                        elif l_tuples == r_tuples:
                            for i in range(0, l_tuples):
                                yield Tuple(None, key)
                        else:
                            for i in range(0, l_tuples):
                                yield Tuple(None, key)
